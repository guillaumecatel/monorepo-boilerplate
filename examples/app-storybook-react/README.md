...
│   ├── style.css
│   ├── vitest.setup.ts
│   ├── i18n/                  # Generated by Paraglide.js
│   └── decorators/
│       └── withTranslations.tsx
├── src/
│   └── Welcome.stories.tsx    # Example story
...
- Deux options de déploiement :
  - **Serveur Node.js (Express)** (inclus, prêt pour Docker/VM/Cloud Run)
  - **Hébergement statique** (Vercel, Netlify, GitHub Pages, S3, etc.)

### Déploiement avec le serveur Express intégré
...
Par défaut, le serveur écoute sur le port 3001 (modifiable via la variable d'environnement `PORT`).
...
Vous pouvez créer une image Docker personnalisée pour servir Storybook avec Express.

### Hébergement statique

Le dossier `storybook-static/` peut être déployé sur n'importe quel hébergeur de fichiers statiques :
...
3. Configurez le dossier de sortie à `apps/<your-app-name>/storybook-static`.

#### Exemple de déploiement sur Netlify

1. Poussez votre repo sur GitHub/GitLab.
2. Connectez le repo à Netlify.
3. Configurez le dossier de build à `apps/<your-app-name>/storybook-static`.
...
**Built with ❤️ using Storybook, React, Paraglide.js, and Tailwind CSS, and the monorepo generator system.**
